    ...
    const pillBtn = (active: boolean) =>
        `flex shrink-0 items-center gap-2.5 rounded-xl px-3.5 py-2.5 text-left text-sm font-bold transition ${
            active
                ...
                : "text-navy-600 hover:bg-white dark:text-navy-200 dark:hover:bg-navy-800"
        }`;
            ...
            <div className="container-page">
                <Reveal className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
                    <div>
                        ...
                        <p className="mt-4 max-w-xl text-bolt-900 dark:text-bolt-800">
                            {t.serviceSubtitle}
                        </p>
                    </div>
                    <a
                ...
                <div className="mt-10 grid gap-8 lg:grid-cols-[260px_1fr]">
                      ...
                      {serviceLang === "en" ? cat.labelEn : cat.labelSw}
                                        ...
                                        <span
                                            className={`ml-auto rounded-full px-1.5 py-0.5 text-[11px] ${
                                                active
                                                    ? "bg-white/20"
                                                    : "bg-navy-200/70 dark:bg-navy-800"
                                            }`}
                                        >
                      {cat.items.length}
                    </span>
                    ...
                    <div className="min-w-0">
                        <div className="flex flex-col gap-3 rounded-3xl border border-navy-200 bg-white p-3 sm:flex-row sm:items-center dark:border-navy-700 dark:bg-navy-800">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    className="w-full rounded-2xl bg-navy-50 py-3.5 pl-11 pr-10 text-sm font-medium outline-none transition focus:bg-navy-100 dark:bg-navy-800 dark:text-white"
                                />
                                {query && (
                                    <button
                                        onClick={() => setQuery("")}
                                        aria-label="Clear search"
                                        className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-1.5 text-bolt-800 hover:bg-navy-200 hover:text-navy-700 dark:hover:bg-navy-700 dark:hover:text-white"
                                    >
                                        <X size={15} />
                                    </button>
                                )}
                ...
                <span className="text-xs font-semibold text-bolt-800">
                  {t.serviceViewLang}
                                ...
                                <div className="flex items-center gap-1 rounded-full bg-navy-100 p-1 dark:bg-navy-800">
                        ...
                        <p className="mt-5 text-sm font-semibold text-bolt-800">
                            {resultCount}{" "}
                            {resultCount === 1
                                ? t.serviceResultsOne
                        ...
                        {resultCount === 0 ? (
                            <div className="mt-4 rounded-3xl border border-dashed border-navy-200 p-12 text-center text-bolt-800 dark:border-navy-700">
                                {t.serviceNoResults}
                            </div>
                        ) : (
                        ...
                        <span className="grid h-9 w-9 place-items-center rounded-xl bg-bolt-500/15 text-bolt-700 dark:text-bolt-300">
                          <Icon size={17} />
                                                ...
                                                <h3 className="font-black text-navy-800 dark:text-white">
                                                    {serviceLang === "en" ? cat.labelEn : cat.labelSw}
                                                </h3>
                                            </div>
                                            <div className="flex flex-wrap gap-2">
                                                {cat.items.map((item) => (
                                                    <span
                                                        key={item.id}
                                                        className="rounded-full border border-navy-200 bg-white px-4 py-2 text-sm font-semibold text-navy-700 transition hover:-translate-y-0.5 hover:border-bolt-400 hover:bg-bolt-50 dark:border-navy-700 dark:bg-navy-800 dark:text-navy-200 dark:hover:bg-bolt-900/30"
                                                    >
                            {serviceLang === "en"
                                ? item.en
                                : item.sw ?? item.en}
                        ...
                        {isBrowsingAll && filteredCategories.length > 4 && (
                            <div className="mt-8">
                                <button
                                    ...
                                    className="flex items-center gap-2 rounded-full border border-navy-200 bg-white px-6 py-3 text-sm font-black text-navy-700 transition hover:border-bolt-400 dark:border-navy-700 dark:bg-navy-800 dark:text-navy-200"
                                >
